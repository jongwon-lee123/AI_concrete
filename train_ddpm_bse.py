...
class Config:
    ...
    # 3.1 phase-assemblage proxy bins for grayscale BSE
    phase_dark_max: int = 85
    phase_mid_max: int = 185
...
def parse_args() -> Config:
    ...
    if not 0 <= cfg.phase_dark_max < cfg.phase_mid_max <= 255:
        raise ValueError("phase bin bounds must satisfy 0 <= phase_dark_max < phase_mid_max <= 255")
    return cfg
...
def compute_batch_stats(
    ...
    for img in imgs_uint8:
        ...
        dark_mask = img <= phase_dark_max
        mid_mask = (img > phase_dark_max) & (img <= phase_mid_max)
        bright_mask = img > phase_mid_max
        dark_frac = float(dark_mask.mean())
        mid_frac = float(mid_mask.mean())
        bright_frac = float(bright_mask.mean())
        phase_dark_fracs.append(dark_frac)
        phase_mid_fracs.append(mid_frac)
        phase_bright_fracs.append(bright_frac)

        phase_probs = np.array([dark_frac, mid_frac, bright_frac], dtype=np.float64)
        phase_probs = phase_probs / np.maximum(phase_probs.sum(), 1e-12)
        phase_entropy_vals.append(float(-(phase_probs * np.log(phase_probs + 1e-12)).sum()))
    ...
    return {
        ...
        "phase_dark_frac": float(np.mean(phase_dark_fracs)) if phase_dark_fracs else 0.0,
        "phase_mid_frac": float(np.mean(phase_mid_fracs)) if phase_mid_fracs else 0.0,
        "phase_bright_frac": float(np.mean(phase_bright_fracs)) if phase_bright_fracs else 0.0,
        "phase_entropy_3bin": float(np.mean(phase_entropy_vals)) if phase_entropy_vals else 0.0,
...
def train(cfg: Config) -> None:
    ...
    for step in range(1, cfg.max_steps + 1):
        ...
        if step % cfg.eval_interval == 0:
            ...
            gen_stats = compute_batch_stats(
                sampled_for_eval,
                cfg.pore_threshold_mode,
                cfg.pore_threshold_fixed,
                cfg.phase_dark_max,
                cfg.phase_mid_max,
            )
            real_stats = compute_batch_stats(
                real_for_eval,
                cfg.pore_threshold_mode,
                cfg.pore_threshold_fixed,
                cfg.phase_dark_max,
